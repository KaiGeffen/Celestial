...
| `server/src/network/websocketServer.ts` | 2026-07-19 | Full audit. Fixed: claimMissionRewards now row-locks (double-claim coin race, same pattern as purchaseItem); deck-name logging no longer throws on invalid card ids; TypedWebSocket dispatch (shared) contains handler throws/rejections so a crafted message can't crash the server process; ActiveGame class→interface. Known/left: guest sign-in trusts uuid by design (provider-account gate holds), sendInitialUserData trusts inventory/missions (existing TODO), initPvp queue race (existing TODO). (Update 2026-07-23: setCosmeticSet ownership IS now validated via playerOwnsCosmeticSet, and all deck-save paths run sanitizedCosmeticSet — that earlier TODO is resolved.) Pre-existing server tsc errors throughout db/ + websocketServer (drizzle insert/update types collapse — likely the old TypeScript version, drizzle needs a newer one) and pveSpecialMatch.ts ctor arity — none from this audit |
...
| `server/src/network/` helpers (sessionToken, googleAuth, steamAuth, cosmeticOwnership, sendUserData, leaderboardServer, matchHistoryServer, usernameAvailabilityServer, referralCountServer) | 2026-07-23 | Full read, **no changes needed** — code is solid. sessionToken (HS256 via node crypto: constant-time compare, lazy secret, always HMAC-verifies so alg-spoofing is moot), googleAuth (verified-email gating), steamAuth, and cosmeticOwnership all check out. Confirmed the cosmetic ownership gate is wired: setCosmeticSet → playerOwnsCosmeticSet, deck saves → sanitizedCosmeticSet (corrected the stale websocketServer-row TODO above). Minor observations judged not worth fixing: analytics logFunnelEvent can reject into fire-and-forget public-endpoint callers (only reaches the global unhandledRejection logger in index.ts — log noise, not a crash); referralCountServer's first query sits outside its try (only matters mid DB-outage); match_history/leaderboard endpoints are unauthenticated by design (any uuid's history incl. decks is public — Kai's choice); cosmeticSet JSON.parse fallbacks omit cardback; steamAuth fetch has no timeout |

### Next up (proposed order)

(client lib/utils/scenes/menus and matchRegions queue complete; server match
subsystem + reward/currency db layer + game-logic controllers + network/ helpers
audited 2026-07-23. Natural next candidates: remaining db files (gameState.ts,
db.ts); ai.ts; or achievementManager.ts + startingInventory/startingCards.
Open design Q still flagged in the updateMatchResult row re: gating
