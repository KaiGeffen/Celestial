...
fs.writeFileSync(
  ...
  JSON.stringify(
    { keywords, cards, subjects, deckSize: MechanicsSettings.DECK_SIZE },
    null,
    2,
  ),
)

// --- Per-card pages: /cards/{slug}/ ---
// Every directory under cards/ except these is a generated card page
// (the root .gitignore encodes the same contract).
const KEEP_DIRS = new Set(['assets', 'maker', 'search', 'community', 'deckbuilder'])
...
console.log(
  `cards assets generated: ${keywords.length} keywords, ${cards.length} cards (+pages), ${subjects.length} subjects`,
)
