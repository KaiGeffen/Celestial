...
| `/cards/`     | [cards/](cards/)         | Card maker, search, deck builder, and community gallery. **Ships as its own image** (own Dockerfile/nginx.conf; see [SPEC](cards/SPEC.md)), not part of the shared `sites` image. Its `/cards/api` prefix routes to the game server. |

Each of these is a plain HTML/CSS/image (the card maker adds a JS canvas renderer).

> **Card tools exception to the self-containment rule:** its `assets/` are
> *generated* from `client/` and `shared/` by `cards/generateAssets.ts`
> (gitignored, regenerated at image build) so its cards always match the game.
> Build it from the **repo root**: `docker build -t cards -f sites/cards/Dockerfile .`
> For local dev, run `npx -y tsx sites/cards/generateAssets.ts` once
...
### Card tools (`/cards/`) — full local setup

The card tools need an extra step or two the other sites don't, because (a) its
per-card pages and `assets/` are **generated**, and (b) its community
gallery/publish/search hit `/cards/api`, which only exists on the game
server.

**1. Generate the assets + per-card game pages (required — they're gitignored):**

```bash
npx -y tsx sites/cards/generateAssets.ts
```

This refreshes `sites/cards/assets/` and writes one static page per real
game card at `sites/cards/{slug}/` (e.g. `/cards/boa/`). A fresh checkout
...
- **Static only** — the maker itself + the generated per-card game pages.
  ...
  (their `/cards/api` calls 404 with no server). Use any static server
  pointed at `sites/`:
  ...
  Then open http://127.0.0.1:4950/cards/maker/ .
...
- **Full test** — adds Publish, community gallery, and search. Run the **game
  ...
  `/cards/api` on a single origin (port **5561**), so the relative API calls
  ...
  Then open http://localhost:5561/cards/maker/ .
...
> **Which port?** Static servers (4950) can't answer `/cards/api`, so use
